...
/* =======================================================================
    GOLD_MVP_V0.5 – FULL EDITION (Snapshot semanal global + Dedup determinístico + Indexing)
   ======================================================================= */

/* =========================
    PARÂMETROS GERAIS
   ========================= */
DECLARE @DESLOC INT = 0; -- DESLOCAMENTO EM DIAS PARA CAPTURAR HISTÓRICO
DECLARE @StartDate DATE = CAST(GETDATE() - 31 - @DESLOC AS DATE); --AO RODAR HISTÓRICO SE ATENTAR A DIAS DO MÊS
DECLARE @EndDate   DATE = CAST(GETDATE() - @DESLOC AS DATE);
DECLARE @VersaoRegra NVARCHAR(20) = N'V0.5_FULL';
...
-- PENALIZAÇÕES
DECLARE @pen_Reatrib  FLOAT = -0.00; DECLARE @pen_Perda    FLOAT = -0.00;
DECLARE @pen_Aband    FLOAT = -0.00; DECLARE @pen_Fora     FLOAT = -0.00;
DECLARE @pen_Onera    FLOAT = -0.00; DECLARE @pen_Teleport FLOAT = -0.00;

/* =========================
    STEP 0 – PREPARAÇÃO SSOT
   ========================= */
...
SELECT R.*, CASE WHEN C.Qtd = 1 THEN 1 ELSE 0 END AS FL_NOME_UNICO
INTO #DIM_TECNICOS_FINAL
FROM #DIM_TECNICOS_RAW R
JOIN CtsNomes C ON C.NOME_TECNICO = R.NOME_TECNICO;

/* =======================================================================
    STEP 1 – #EVT (AGRUPAMENTO POR OS)
   ======================================================================= */
DROP TABLE IF EXISTS #EVT;
SELECT
  P.NUM_OS, P.USUARIO,
  MAX(COALESCE(T_UNI.EMPRESA, OP.EMPRESA, T_DUP.EMPRESA, 'EMPRESA NAO IDENTIFICADA')) AS EMPRESA_OS,
  MAX(COALESCE(T_UNI.UF_TEC, T_DUP.UF_TEC, OP.UF_EVENTO, 'UF NAO IDENTIFICADA')) AS UF_TEC,
  MAX(COALESCE(T_UNI.REGIONAL, OP.REGIONAL_EVENTO, 'REGIONAL NAO IDENTIFICADA')) AS REGIONAL_OS,
  MAX(COALESCE(T_UNI.TIPO_TEC, T_DUP.TIPO_TEC, 'TIPO NAO IDENTIFICADO')) AS TIPO_TEC,
  MIN(CASE WHEN P.TIPO_PROC = 'ATRIBUIÇÃO' THEN P.DATA END) AS DT_ATRIB,
  MIN(CASE WHEN P.TIPO_PROC = 'ACEITE' THEN P.DATA END) AS DT_ACEITE,
  MIN(CASE WHEN P.TIPO_PROC IN ('ESTOU NO SITE','ESTOU NA FALHA') THEN P.DATA END) AS DT_SITE_VALIDO,
  MIN(CASE WHEN P.TIPO_PROC = 'ESTOU NO SITE (FORA DE ALCANCE)' THEN P.DATA END) AS DT_SITE_FORA,
  MIN(CASE WHEN P.TIPO_PROC = 'PRE-BAIXA' THEN P.DATA END) AS DT_PREBAIXA,
  MAX(P.DATA) AS DT_ULTIMO_MOVIMENTO_OS,
  MAX(CASE WHEN P.TIPO_PROC = 'PAUSA' THEN 1 ELSE 0 END) AS FL_TEVE_PAUSA,
  MAX(CASE WHEN P.TIPO_PROC = 'ACEITE' AND P.GEO_PROC IS NOT NULL AND P.GEO_PROC.STGeometryType()='Point' THEN TRY_CAST(P.GEO_PROC.Lat AS FLOAT) END) AS LAT_ACEITE,
  MAX(CASE WHEN P.TIPO_PROC = 'ACEITE' AND P.GEO_PROC IS NOT NULL AND P.GEO_PROC.STGeometryType()='Point' THEN TRY_CAST(P.GEO_PROC.Long AS FLOAT) END) AS LONG_ACEITE,
  MAX(CASE WHEN P.TIPO_PROC IN ('ESTOU NO SITE','ESTOU NA FALHA') AND P.GEO_PROC IS NOT NULL AND P.GEO_PROC.STGeometryType()='Point' THEN TRY_CAST(P.GEO_PROC.Lat AS FLOAT) END) AS LAT_SITE,
  MAX(CASE WHEN P.TIPO_PROC IN ('ESTOU NO SITE','ESTOU NA FALHA') AND P.GEO_PROC IS NOT NULL AND P.GEO_PROC.STGeometryType()='Point' THEN TRY_CAST(P.GEO_PROC.Long AS FLOAT) END) AS LONG_SITE
INTO #EVT
FROM GENESIS.dbo.TBL_PROC AS P
LEFT JOIN #DIM_TECNICOS_FINAL T_UNI
       ON T_UNI.NOME_TECNICO = P.USUARIO AND T_UNI.FL_NOME_UNICO = 1
LEFT JOIN GENESIS.dbo.TBL_OS_PAGAMENTO OP
       ON OP.NUM_OS = P.NUM_OS
LEFT JOIN #DIM_TECNICOS_FINAL T_DUP
       ON T_DUP.NOME_TECNICO = P.USUARIO AND T_DUP.FL_NOME_UNICO = 0 AND T_DUP.EMPRESA = OP.EMPRESA
WHERE P.DATA >= @StartDate AND P.DATA < DATEADD(DAY, 1, @EndDate)
GROUP BY P.NUM_OS, P.USUARIO;

/* =========================
    DATA REFERÊNCIA GLOBAL DA CARGA (SEMANA)
    A MAIOR DT_ULTIMO_MOVIMENTO_OS DA CARGA; FALLBACK = @DataExecucao
   ========================= */
DECLARE @Carga_Ref_Data DATETIME;
SELECT @Carga_Ref_Data = COALESCE(MAX(DT_ULTIMO_MOVIMENTO_OS), @DataExecucao)
FROM #EVT;

/* =========================
    STEP 2 A 5 – FLAGS & AJUSTES
   ========================= */
  ...
  CASE WHEN E.FL_ATRIB = 1 AND E.FL_ACEITE = 0
        AND NOT EXISTS (
            ...
            WHERE P.NUM_OS = E.NUM_OS
              AND P.DATA >= @StartDate AND P.DATA < DATEADD(DAY, 1, @EndDate)
              AND P.TIPO_PROC IN ('ACEITE','ESTOU NO SITE','ESTOU NA FALHA','PRÉ-BAIXA')
        )
...
/* =========================
    STEP 6 – DEFLATORES + GEO
   ========================= */
DROP TABLE IF EXISTS #DEF_FORA_SITE;
SELECT A.NUM_OS, A.USUARIO,
       CASE WHEN A.FL_SITE_FORA = 1
             AND NOT EXISTS(SELECT 1 FROM #EVT B WHERE B.NUM_OS = A.NUM_OS AND B.USUARIO = A.USUARIO AND B.DT_SITE_VALIDO IS NOT NULL)
...
SELECT A.NUM_OS, A.USUARIO,
       CASE WHEN A.FL_ATRIB = 1 AND A.FL_ACEITE = 0
                 AND NOT EXISTS(SELECT 1 FROM GENESIS.dbo.TBL_PROC P WHERE P.NUM_OS = A.NUM_OS AND P.TIPO_PROC IN ('ACEITE','ESTOU NO SITE','ESTOU NA FALHA','PRÉ-BAIXA'))
...
SELECT A.NUM_OS, A.USUARIO,
       CASE WHEN A.DT_ACEITE IS NOT NULL AND A.DT_PREBAIXA IS NULL
                 AND EXISTS(SELECT 1 FROM #EVT B WHERE B.NUM_OS = A.NUM_OS AND B.USUARIO <> A.USUARIO AND COALESCE(B.DT_ACEITE, B.DT_SITE_VALIDO, B.DT_PREBAIXA) > A.DT_ACEITE)
...
SELECT A.NUM_OS, A.USUARIO,
       CASE WHEN A.DT_ATRIB IS NOT NULL AND A.DT_ACEITE IS NULL
                 AND EXISTS(SELECT 1 FROM #EVT B WHERE B.NUM_OS = A.NUM_OS AND B.USUARIO <> A.USUARIO AND B.DT_ACEITE > A.DT_ATRIB)
...
SELECT A.NUM_OS, A.USUARIO,
       CASE WHEN A.LAT_ACEITE IS NOT NULL AND A.LAT_SITE IS NOT NULL AND DATEDIFF(MINUTE, A.DT_ACEITE, A.DT_SITE_VALIDO) >= 0
             THEN CASE WHEN (geography::Point(A.LAT_ACEITE, A.LONG_ACEITE, 4326).STDistance(geography::Point(A.LAT_SITE, A.LONG_SITE, 4326)) / 1000.0)
       ...
       CASE WHEN A.LAT_ACEITE IS NOT NULL AND A.LAT_SITE IS NOT NULL
             THEN geography::Point(A.LAT_ACEITE, A.LONG_ACEITE, 4326).STDistance(geography::Point(A.LAT_SITE, A.LONG_SITE, 4326))
...
SELECT
    ...
    CASE WHEN E.DT_ACEITE IS NULL OR E.DT_PREBAIXA IS NULL OR (DATEDIFF(MINUTE, E.DT_ACEITE, E.DT_PREBAIXA)) < @Treshold_Min THEN 0.00
         ...
         ELSE @Pt_Tier5 END AS PONTOS_TEMPO_GERADOS_OS,
    ...
    CASE WHEN E.DT_ACEITE IS NOT NULL AND E.DT_PREBAIXA IS NULL OR (DATEDIFF(MINUTE, E.DT_ACEITE, E.DT_PREBAIXA)) < @Treshold_Min THEN 1 ELSE 0 END AS concluidos_tempo_pendente
INTO #TEMPOS
FROM #EVT E;

/* =======================================================================
    STEP 9 – AGREGAR TUDO POR TÉCNICO
   ======================================================================= */
...
DROP TABLE IF EXISTS #BASE_WINSOR;
SELECT DISTINCT * INTO #BASE_WINSOR FROM #BASE_TEC_RAW;
...
/* =======================================================================
    STEP 14 – GERAÇÃO DA TABELA FINAL (FULL SCHEMA) [UPDATED]
   ======================================================================= */
DROP TABLE IF EXISTS #LOGIN_CHECK;
SELECT USUARIO, MAX(DATA) AS DT_ULTIMO_LOGIN_GERAL, 1 AS FL_LOGIN_RECENTE
INTO #LOGIN_CHECK
FROM GENESIS.dbo.TBL_PROC
WHERE DATA >= DATEADD(DAY, -@Days_Login_Window, CAST(GETDATE() AS DATE))
GROUP BY USUARIO;
...
SELECT
    ...
    CAST(FLOOR(
        CASE WHEN (SELECT MAX(v) FROM (VALUES (W.ATRIB_AJUST), (W.ACEITE_AJUST), (W.SITE_TOTAL_AJUST)) AS t(v)) > 0
             ...
             ELSE 0 END * 100) / 100.0 AS DECIMAL(10,2)) AS NOTA_PRODUTIVIDADE,

    CAST(FLOOR(
        CASE WHEN W.ACEITE_AJUST > 0
             THEN (S.PTS_TEMPO / NULLIF(W.ACEITE_AJUST * 1.0, 0)) * 100.0
             ELSE 0 END * 100) / 100.0 AS DECIMAL(10,2)) AS NOTA_TEMPO,

    CAST(FLOOR(S.PTS_DEFLACAO * 100) / 100.0 AS DECIMAL(10,2)) AS NOTA_DEFLATOR,
    ...
    CAST(FLOOR(
        ( ((CASE WHEN (SELECT MAX(v) FROM (VALUES (W.ATRIB_AJUST), (W.ACEITE_AJUST), (W.SITE_TOTAL_AJUST)) AS t(v)) > 0
          ...
          + S.PTS_DEFLACAO * 100) / 100.0 AS DECIMAL(10,2)) AS NOTA_FINAL,

    CAST(FLOOR(S.PTS_PRODUTIVIDADE * 100) / 100.0 AS DECIMAL(10,2)) AS PONTUACAO_PRODUTIVIDADE,
    CAST(FLOOR(S.PTS_TEMPO * 100) / 100.0 AS DECIMAL(10,2)) AS PONTUACAO_TEMPO,
    CAST(FLOOR(S.PTS_DEFLACAO * 100) / 100.0 AS DECIMAL(10,2)) AS PONTUACAO_DEFLATOR,
    CAST(FLOOR((S.PTS_PRODUTIVIDADE + S.PTS_TEMPO + S.PTS_DEFLACAO) * 100) / 100.0 AS DECIMAL(10,2)) AS PONTUACAO_FINAL_TOTAL,
...
-- /*antigo key score*/
--     CAST(FLOOR(CASE WHEN ((S.PTS_PRODUTIVIDADE / @Meta_Pontos_Max) * 1000) > 1000
--                     THEN 1 ELSE (S.PTS_PRODUTIVIDADE / @Meta_Pontos_Max) END * @Peso_Dimensao_Prod * 100) / 100.0 AS DECIMAL(10,2)) AS KEY_SCORE_PROD,

--     CAST(FLOOR(CASE WHEN ((S.PTS_TEMPO / @Meta_Pontos_Max) * 1000) > 1000
--                     THEN 1 ELSE (S.PTS_TEMPO / @Meta_Pontos_Max) END * @Peso_Dimensao_Tempo * 100) / 100.0 AS DECIMAL(10,2)) AS KEY_SCORE_TIME,

--     CAST(FLOOR(((CASE WHEN ((S.PTS_PRODUTIVIDADE / @Meta_Pontos_Max) * 1000) > 1000
--                       THEN 1 ELSE (S.PTS_PRODUTIVIDADE / @Meta_Pontos_Max) END * @Peso_Dimensao_Prod)
--                + (CASE WHEN ((S.PTS_TEMPO / @Meta_Pontos_Max) * 1000) > 1000
--                       THEN 1 ELSE (S.PTS_TEMPO / @Meta_Pontos_Max) END * @Peso_Dimensao_Tempo)) * 1000 * 100) / 100.0 AS DECIMAL(10,2)) AS KEY_SCORE,

    /* Novas colunas – Iguais para TODOS da carga */
    CONCAT(CONVERT(char(6), @Carga_Ref_Data, 112),
           RIGHT('00' + CAST(DATEPART(ISO_WEEK, @Carga_Ref_Data) AS varchar(2)), 2)
    ) AS anomessemana,
...
/* =========================
    STEP 15 – PERSISTÊNCIA [UPDATED]
   ========================= */
IF OBJECT_ID('GENESIS.PROD.TBL_RANK_TEC', 'U') IS NULL
BEGIN
    -- Cria tabela com schema incluindo as novas colunas
    SELECT * INTO GENESIS.PROD.TBL_RANK_TEC FROM #SILVER_FINAL;
END
 ELSE
BEGIN
    -- Migração de schema: adiciona colunas novas se não existirem
    IF NOT EXISTS (
        SELECT 1
        FROM GENESIS.sys.columns c
        JOIN GENESIS.sys.objects o ON o.object_id = c.object_id
        WHERE o.name = 'TBL_RANK_TEC'
          AND o.schema_id = SCHEMA_ID('PROD')
          AND c.name = 'anomessemana'
    )
        --ALTER TABLE GENESIS.PROD.TBL_RANK_TEC ADD anomessemana VARCHAR(8) NULL);
    ...
    IF NOT EXISTS (
        ...
        WHERE o.name = 'TBL_RANK_TEC'
          AND o.schema_id = SCHEMA_ID('PROD')
          AND c.name = 'max_data_anomes'
    )
        --ALTER TABLE GENESIS.PROD.TBL_RANK_TEC ADD max_data_anomes DATETIME NULL;

    -- Adiciona a coluna TIPO_TEC se não existir
    IF NOT EXISTS (
        SELECT 1
        FROM GENESIS.sys.columns c
        JOIN GENESIS.sys.objects o ON o.object_id = c.object_id
    ...
    -- BEGIN
    --     ALTER TABLE GENESIS.PROD.TBL_RANK_TEC ADD TIPO_TEC VARCHAR(10) NULL;
    -- END

    -- Persistência: inserimos sem apagar por DIA
    INSERT INTO GENESIS.PROD.TBL_RANK_TEC
    SELECT * FROM #SILVER_FINAL;

    -- Deduplicação por semana (desempate determinístico):
    ;WITH d AS (
        SELECT
            USUARIO,
            anomessemana,
            ...
            ROW_NUMBER() OVER (
                ...
                ORDER BY PROCESSADO_EM DESC,     -- mais recente
                         max_data_anomes DESC,   -- reforça a semana da carga
                         VERSAO DESC,            -- versão mais nova primeiro
                         USUARIO ASC             -- tiebreaker estável
            ) AS rn
...
/* =========================
    INDEXAÇÃO (idempotente) — checando no catálogo do banco GENESIS
   ========================= */

-- -- CLUSTERED: (max_data_anomes DESC, KEY_SCORE DESC)
-- IF NOT EXISTS (
--     SELECT 1
--     FROM GENESIS.sys.indexes i
--     JOIN GENESIS.sys.objects o ON o.object_id = i.object_id
--     WHERE o.name = 'TBL_RANK_TEC'
--       AND o.schema_id = SCHEMA_ID('PROD')
--       AND i.name = 'CX_TBL_RANK_TEC__DataScore'
-- )
-- BEGIN
--     DECLARE @ExistingClustered sysname;
--     SELECT @ExistingClustered = i.name
--     FROM GENESIS.sys.indexes i
--     JOIN GENESIS.sys.objects o ON o.object_id = i.object_id
--     WHERE o.name = 'TBL_RANK_TEC'
--       AND o.schema_id = SCHEMA_ID('PROD')
...
--     IF @ExistingClustered IS NOT NULL AND @ExistingClustered <> 'CX_TBL_RANK_TEC__DataScore'
--     BEGIN
--         -- Troca o clustered existente pela ordenação desejada
--         EXEC('CREATE CLUSTERED INDEX CX_TBL_RANK_TEC__DataScore
--               ON GENESIS.PROD.TBL_RANK_TEC (max_data_anomes DESC, KEY_SCORE DESC)
--               WITH (DROP_EXISTING = ON);');
--     END
--     ELSE IF @ExistingClustered IS NULL
--     BEGIN
--         EXEC('CREATE CLUSTERED INDEX CX_TBL_RANK_TEC__DataScore
--               ON GENESIS.PROD.TBL_RANK_TEC (max_data_anomes DESC, KEY_SCORE DESC)
...
-- -- NONCLUSTERED 1: (USUARIO, anomessemana) com INCLUDE
-- IF NOT EXISTS (
--     SELECT 1
--     FROM GENESIS.sys.indexes i
--     JOIN GENESIS.sys.objects o ON o.object_id = i.object_id
--     WHERE o.name = 'TBL_RANK_TEC'
--       AND o.schema_id = SCHEMA_ID('PROD')
--       AND i.name = 'IX_TBL_RANK_TEC__UsuarioSemana'
-- )
-- BEGIN
--     EXEC('CREATE NONCLUSTERED INDEX IX_TBL_RANK_TEC__UsuarioSemana
--           ON GENESIS.PROD.TBL_RANK_TEC (USUARIO ASC, anomessemana ASC)
--           INCLUDE (PROCESSADO_EM, max_data_anomes, KEY_SCORE);');
-- END;

-- -- NONCLUSTERED 2: (anomessemana ASC, max_data_anomes DESC) com INCLUDE
-- IF NOT EXISTS (
--     SELECT 1
--     FROM GENESIS.sys.indexes i
--     JOIN GENESIS.sys.objects o ON o.object_id = i.object_id
--     WHERE o.name = 'TBL_RANK_TEC'
--       AND o.schema_id = SCHEMA_ID('PROD')
--       AND i.name = 'IX_TBL_RANK_TEC__AnoMesData'
-- )  
-- BEGIN
--     EXEC('CREATE NONCLUSTERED INDEX IX_TBL_RANK_TEC__AnoMesData
--           ON GENESIS.PROD.TBL_RANK_TEC (anomessemana ASC, max_data_anomes DESC)
--           INCLUDE (USUARIO, KEY_SCORE);');
-- END;

/* =========================
    RETORNOS
   ========================= */
SELECT 'SUCESSO' AS STATUS, COUNT(*) AS TOTAL_LINHAS FROM #SILVER_FINAL;

SELECT * FROM GENESIS.PROD.TBL_RANK_TEC;

-- DROP TABLE IF EXISTS GENESIS.PROD.TBL_RANK_TEC
