...
DECLARE @Carga_Ref_Data DATETIME = @EndDate;
...
CREATE INDEX IX_PROC_PERIODO_OS_USER          ON #PROC_PERIODO (NUM_OS, USUARIO, RE);

CREATE INDEX IX_PROC_PERIODO_EVENT_SEEK       ON #PROC_PERIODO (NUM_OS, USUARIO, RE, TIPO_PROC_NORM, DATA);

CREATE INDEX IX_PROC_PERIODO_USER_DATE        ON #PROC_PERIODO (USUARIO, RE, DATA);
...
SELECT DISTINCT NUM_OS, RE, USUARIO
...
CREATE CLUSTERED INDEX CX_DU ON #DU (NUM_OS, RE, USUARIO);
...
GROUP BY P.NUM_OS, P.USUARIO, P.RE;



CREATE CLUSTERED INDEX CX_EVT_MIN ON #EVT_MIN (NUM_OS, USUARIO, RE);
...
CREATE CLUSTERED INDEX CX_ACEITE_GEO ON #ACEITE_GEO (NUM_OS, USUARIO, RE);
...
CREATE CLUSTERED INDEX CX_SITE_OK_GEO ON #SITE_OK_GEO (NUM_OS, USUARIO, RE);
...
CREATE CLUSTERED INDEX CX_PAUSA_FLAG ON #PAUSA_FLAG (NUM_OS, USUARIO, RE);
...
LEFT JOIN #DIM_TECNICOS_FINAL T_UNI ON T_UNI.RE = DU.RE AND T_UNI.FL_NOME_UNICO = 1

LEFT JOIN #DIM_TECNICOS_FINAL T_DUP ON T_DUP.RE = DU.RE AND T_DUP.FL_NOME_UNICO = 0 AND T_DUP.EMPRESA = #OP_ONE.EMPRESA
...
SELECT
  ...
  NUM_OS, USUARIO, RE, EMPRESA_OS, REGIONAL_OS, UF_TEC, TIPO_TEC,
...
SELECT A.NUM_OS, A.USUARIO, A.RE,
...
SELECT
  ...
  E.NUM_OS, E.USUARIO, E.RE, E.DT_PREBAIXA,
...
FROM (SELECT DISTINCT USUARIO, RE, NUM_OS, EMPRESA_OS, REGIONAL_OS, DT_PREBAIXA FROM #FLAGS) T
...
FROM (SELECT DISTINCT USUARIO, RE, NUM_OS FROM #FLAGS) U
...
SELECT
  ...
  U.USUARIO, U.RE, U.UF, U.TIPO_TEC,
...
FROM (SELECT DISTINCT USUARIO, RE, UF_TEC AS UF, TIPO_TEC FROM #FLAGS) U
...
SELECT
  ...
  EMPRESA_PRINCIPAL AS EMPRESA, W.REGIONAL_PRINCIPAL AS REGIONAL, W.UF,
